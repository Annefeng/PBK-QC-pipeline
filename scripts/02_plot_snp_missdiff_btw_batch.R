...
dat_missdiff <- as.data.frame(matrix(nrow=length(snplist), ncol=nrow(uniq_batch_pair)))
names(dat_missdiff) <- paste0(uniq_batch_pair[,1],".",uniq_batch_pair[,2])
dat_missdiff <- cbind(snplist, dat_missdiff)
...
for ( i in 1:nrow(uniq_batch_pair) ) {
    ...
    dat_missdiff[,paste0(x,".",y)] <- comb$F_MISS.diff
...
fwrite(dat_missdiff, "snp_missRateDiff_btw_batch.tsv", quote=F, col.names=T, row.names=F, 
    sep="\t")
